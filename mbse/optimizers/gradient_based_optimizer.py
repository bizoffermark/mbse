...
class GradientBasedOptimizer(DummyOptimizer):
    ...
    def optimize(self, func, rng=None):
        ...
        solver = GradientDescent(fun=lambda x: -func(self.clip_action(x)), maxiter=self.num_steps, jit=False)
        run = jax.jit(jax.vmap(lambda x: solver.run(init_params=x), in_axes=0, out_axes=0))
        res = run(samples)
        params, state = res
        params = self.clip_action(params)
        result = state.error
        best_idx = np.argsort(result, axis=0)[0]
        best_value = result[best_idx]
        best_sequence = params[best_idx]

        # def loss_fn(action):
        #     values = vmap(func)(action)
        #     return -values.mean(), values
        # grad_func = jax.value_and_grad(loss_fn, has_aux=True)
        # #y, vjp = jax.vjp(loss_fn)
